...
{
    private readonly Dictionary<FilteringOperators, (Func<object?, Expression> obtainFilterExpression, Func<string?, object?>? filterValueConverter)> _filters = [];
    ...
    public CustomFiltersBuilder<TEntity> WithFilter(FilteringOperators @operator, Func<string?, Expression<Func<TEntity, bool>>> obtainFilterExpression)
    {
        ...
        Func<TFilterValue?, Expression<Func<TEntity, bool>>> obtainFilterExpression)
    {
        ...
        _filters[@operator] = (filterValue => obtainFilterExpression((TFilterValue?)filterValue), filterValue => filterValueConverter.ConvertFrom(filterValue));
        return this;
    }

    /// <summary>
    /// Adds a custom filter with the specified filtering operator, filter value converter function.
    /// </summary>
    /// <typeparam name="TFilterValue">The type of the filter value.</typeparam>
    /// <param name="operator">The filtering operator associated with the filter.</param>
